...
async def main():
    ...
    for i, doc in enumerate(documents):
        # 生成简化的向量（实际应用中应该使用真实的嵌入服务）
        vector = [0.1 * (i + 1) + 0.01 * j for j in range(vector_size)]
        vectors.append(
            VectorUpload(
                id=str(uuid.uuid4()),
                vector=vector,
    ...
    # 生成简化的查询向量
    query_vector = [0.15 + 0.01 * j for j in range(vector_size)]
    try:
